...
internal class DataWatcherItem12105 : DataWatcherItem {
    ...
    fun getOptionalDataWatcher(index: Int, value: Optional<*>): DataWatcher.Item<out Optional<out Any?>?> {
        return when (value.getOrNull()) {
            is IChatBaseComponent -> {
                DataWatcher.Item(DataWatcherRegistry.OPTIONAL_COMPONENT.createAccessor(index), value as Optional<IChatBaseComponent>)
            }
    ...
    override fun getDataWatcherItem(index: Int, value: Any): Any {
        if (value is net.minecraft.server.v1_16_R3.DataWatcher.Item<*>) return value
        ...
        return when (value) {
            ...
            is IBlockData -> getDataWatcherItem(index, value)
