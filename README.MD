...
PacketFactory 是一个 Minecraft 服务器数据包工厂库，提供跨版本的统一 API 来创建各种 NMS 网络数据包。
...
```kotlin
taboolib {
    relocate("org.craft.packetfactory", "${project.group}.libs.packetfactory")
}
```
...
- 库只负责创建数据包对象，发送需配合 TabooLib 或 ProtocolLib
